...
std::string FileData::getlaunchCommand(LaunchGameOptions& options, bool includeControllers)
{
	...
	bool useGuns = SystemConf::getInstance()->getBool("global.use_guns", true);
	bool useWheels = SystemConf::getInstance()->getBool("global.use_wheels", true);
	std::string controllersConfig = InputManager::getInstance()->configureEmulators(isLightGunGame() && useGuns, isWheelGame() && useWheels);

	if (gameToUpdate->isLightGunGame() && useGuns)
		controllersConfig = controllersConfig + "-lightgun ";

        if (gameToUpdate->isWheelGame() && useWheels)
