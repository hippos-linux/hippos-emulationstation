...
GuiPad2key::GuiPad2key(Window* window, Pad2key ktp)
	: GuiSettings(window, _("PAD2KEY"), true)
{
	m_ktp = ktp;
	m_devices = ApiSystem::getInstance()->getPad2keyDevices(m_ktp.config);
	m_need_save = false;
	...
	onFinalize([this, window]
	{
		if (this->m_need_save)
		{
			...
			ApiSystem::getInstance()->savePad2key(this->m_ktp, this->m_devices);
			window->displayNotificationMessage(_("UNPLUG AND REPLUG THE DEVICE TO APPLY"));
		}
	});
}

void GuiPad2key::loadActivePage(const std::string& device_path)
{
	mMenu.clear();
	mMenu.clearButtons();
...
Pad2keyDevice* GuiPad2key::getJoystickDevice(int n) 
{
	unsigned int x = 0;
...
Pad2keyDevice* GuiPad2key::getHotkeyDevice() 
{
	for (unsigned int i = 0; i < m_devices.size(); i++) 
...
int GuiPad2key::countJoystickDevice(const std::string& type) 
{
	int n = 0;
...
void GuiPad2key::loadPlayerPage(int n, const std::string& device_path)
{
	std::vector<Pad2keyInputStructure> GUI_INPUT_CONFIG_LIST =
	{
	...
	Pad2keyDevice* device = getJoystickDevice(n - 1);
...
void GuiPad2key::declareEvKey(Window* window, const std::string& device_path, const std::function<void(std::string)>& func)
{
...
std::string GuiPad2key::getNameForInput(Pad2keyDevice* device, std::string value) 
{
	for (unsigned int i = 0; i < device->keys.size(); i++) 
...
void GuiPad2key::loadHotkeysPage(const std::string& device_path)
{
	...
	Pad2keyDevice* device = getHotkeyDevice();
	if (device == nullptr) 
		return;

	Window* window = mWindow;

	std::vector<Pad2keyKey> key_values = ApiSystem::getInstance()->getPad2keyKeyValues();
	auto theme = ThemeData::getMenuTheme();
...
std::vector<HelpPrompt> GuiPad2key::getHelpPrompts()
{
...
void GuiPad2key::setDeviceValue(Pad2keyDevice* ktp, std::string key, std::string value, Window* window, const std::string& device_path) 
{
