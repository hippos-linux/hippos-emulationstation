...
GuiInstall::GuiInstall(Window* window, std::function<std::pair<std::string,int>(BusyComponent*)> op)
    : GuiComponent(window), mBusyAnim(window), mOp(op)
{
...
GuiInstall::GuiInstall(Window* window, std::string storageDevice, std::string architecture)
    : GuiInstall(window, [storageDevice, architecture](BusyComponent* ui) {
          return ApiSystem::getInstance()->installSystem(ui, storageDevice, architecture);
      })
{
}
...
void GuiInstall::update(int deltaTime)
{
    ...
    if (mState == 2) {
        window->pushGui(new GuiMsgBox(window, _("FINISHED"), _("OK"), [this] { mState = -1; }));
        mState = 0;
    }
    if (mState == 3) {
...
{
    std::pair<std::string,int> status = mOp(&mBusyAnim);
    if (status.second == 0)
        onInstallOk();
    else
        onInstallError(status);
}
