	...
	mMenu(window, game->getName()), mReloadAll(false)
{
	...
	// Version selector for parent-child game groups
	{
		...
		if (!childGames.empty())
		{
			...
			versionList->addEx(getVersionLabel(rootParent), getVersionBadges(rootParent), rootParent, sourceGame == rootParent);
			for (auto child : childGames)
				versionList->addEx(getVersionLabel(child), getVersionBadges(child), child, sourceGame == child);

			versionList->setSelectedChangedCallback([this](FileData* selected)
			{
			...
			mMenu.addWithLabel(_("VERSION"), versionList);
		}
	}
