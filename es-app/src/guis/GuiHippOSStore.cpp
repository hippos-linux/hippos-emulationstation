...
GuiHippOSStore::GuiHippOSStore(Window* window)
...
GuiHippOSStore::~GuiHippOSStore()
{
	ContentInstaller::UnregisterNotify(this);
}

void GuiHippOSStore::OnContentInstalled(int contentType, std::string contentName, bool success)
{
	if (contentType == ContentInstaller::CONTENT_STORE_INSTALL || contentType == ContentInstaller::CONTENT_STORE_UNINSTALL)
		mReloadList = 2;
}

void GuiHippOSStore::update(int deltaTime)
{
...
void GuiHippOSStore::onSizeChanged()
{
...
void GuiHippOSStore::centerWindow()
{
...
void GuiHippOSStore::loadList(bool updatePackageList, bool restoreIndex)
{
	...
	for (auto package : mPackages)
	{
		...
		auto grid = std::make_shared<GuiHippOSStoreEntry>(mWindow, package, hasImages);
...
void GuiHippOSStore::loadPackagesAsync(bool updatePackageList)
{
	Window* window = mWindow;

	mWindow->pushGui(new GuiLoading<std::vector<PacmanPackage>>(mWindow, _("PLEASE WAIT"),
		[this, updatePackageList](auto gui)
		{
...
void GuiHippOSStore::processPackage(PacmanPackage package)
{	
...
bool GuiHippOSStore::input(InputConfig* config, Input input)
{
...
void GuiHippOSStore::showSearch()
{
...
std::vector<HelpPrompt> GuiHippOSStore::getHelpPrompts()
{
...
GuiHippOSStoreEntry::GuiHippOSStoreEntry(Window* window, PacmanPackage& entry, bool withPreviewImage) :
	ComponentGrid(window, Vector2i(5, 4))
{
	...
	if (!entry.repository.empty() && entry.repository != "hippos")
...
void GuiHippOSStoreEntry::setColor(unsigned int color)
{
...
void GuiHippOSStoreEntry::update(int deltaTime)
{
