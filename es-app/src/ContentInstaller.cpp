...
void ContentInstaller::threadUpdate()
{
	...
	while (true)
	{
		...
		{
			updateStatus = ApiSystem::getInstance()->installStorePackage(data.second, [this](const std::string info)
			{
				updateNotificationComponentContent(info);
			});
		...
		{
			updateStatus = ApiSystem::getInstance()->uninstallStorePackage(data.second, [this](const std::string info)
			{
				updateNotificationComponentContent(info);
			});
