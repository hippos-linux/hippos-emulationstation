	...
	: mDeviceId(deviceId), mDeviceIndex(deviceIndex), mDeviceName(deviceName), mDeviceGUID(deviceGUID), mDeviceNbButtons(deviceNbButtons), mDeviceNbHats(deviceNbHats), mDeviceNbAxes(deviceNbAxes), mDevicePath(devicePath)
{
	mBatteryLevel = -1;
	mIsWheel = false;
	mWheelRotationAngle = 0;
	mDeviceParentSysPath = "";
#ifdef HAVE_UDEV
	mIsWheel = isWheel(devicePath);
	if (mIsWheel)
		mWheelRotationAngle = wheelRotationAngle(devicePath);
...
int InputConfig::wheelRotationAngle(const std::string path) {
	struct udev *udev;
	struct udev_list_entry *devs = NULL;
	struct udev_list_entry *item = NULL;
	int res = 0;
	...
	if (udev != NULL)
	  {
		...
		const char* devnode;
		  ...
		  std::string strdevnode = devnode;
		  if(strdevnode == path) {
		    const char* rotation = udev_device_get_property_value(dev, "WHEEL_ROTATION_ANGLE");
		    if (rotation != NULL)
		      res = std::atoi(rotation);
		  }
	    	}
	    	udev_device_unref(dev);
	      }
