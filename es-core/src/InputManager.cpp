...
{	
	...
	for (auto tt : mapArray)
	{
		...
		if (input.configured)
		{
			// HIPPOS : Uncomment the next line using a patch to compute the code
			// input.computeCode();
...
std::map<int, InputConfig*> InputManager::computePlayersConfigs(bool preferWheels)
{
	std::unique_lock<std::mutex> lock(mJoysticksLock);
	...
	std::sort(availableConfigured.begin(), availableConfigured.end(), [preferWheels](InputConfig * a, InputConfig * b) -> bool {
		if (preferWheels && a->isWheel() != b->isWheel())
			return a->isWheel() > b->isWheel();
		return a->getSortDevicePath() < b->getSortDevicePath();
	});
...
std::string InputManager::configureEmulators(bool lightgun, bool wheel) {
  std::map<int, InputConfig*> playerJoysticks = computePlayersConfigs(wheel);
  ...
  for (int player = 0; player < MAX_PLAYERS; player++) {
    ...
    if(playerInputConfig != NULL){
      ...
      command << " -p" << player+1 << "devicepath \"" << playerInputConfig->getDevicePath() << "\"";
      ...
      command << " -p" << player+1 << "iswheel "    << (playerInputConfig->isWheel() ? 1 : 0);
      command << " -p" << player+1 << "wheelrotationangle " << playerInputConfig->getWheelRotationAngle();
      command << " ";
    }
  }
